...
# touch jenkins or devops-app. Assignment 5. Everything Prometheus/Grafana
# end up configured with comes from files in this repo (observability/values.yaml,
# rules/, dashboards/, networkpolicies.yaml) - nothing here is a manual UI step.
#
# Idempotent: safe to re-run. kubectl apply/helm upgrade --install all no-op
# cleanly on an existing, unchanged install - same pattern as
# jenkins/scripts/install-jenkins.sh.
#
# Run helm/devops-app's next upgrade (or jenkins/scripts/install-jenkins.sh,
# for its new ServiceMonitor) only AFTER this script - both reference the
# ServiceMonitor/PodMonitor/PrometheusRule CRDs this chart installs, and
# `helm upgrade` on either fails outright ("no matches for kind
# ServiceMonitor") if those CRDs don't exist yet.

CLUSTER_NAME="devops-cluster"
AWS_REGION="us-east-1"
# Pinned against Artifact Hub / the prometheus-community index at the time
# it was checked - not "latest", same reasoning as JENKINS_CHART_VERSION in
# install-jenkins.sh.
...
if kubectl get secret grafana-admin-credentials -n observability >/dev/null 2>&1; then
    ...
    # (jenkins/scripts/configure-jenkins.sh's [1/5] step). Retrieve it later with:
...
# Unlike Jenkins' own slack-webhook-url Secret (which is skipped entirely
# when unset - notifySlack.groovy tolerates a missing credential),
# observability/values.yaml's alertmanager.alertmanagerSpec.secrets always
# names this Secret, and Alertmanager's Pod fails to start at all
# (CreateContainerConfigError) if a Secret it's told to mount doesn't exist.
# So this Secret always gets created, real value or a placeholder - a
# placeholder just means Slack delivery quietly fails (logged, retried,
# never crashes the Pod) until a real webhook URL is supplied, same
# "optional, degrades gracefully" spirit as the Jenkins pattern, just
# implemented the one way that doesn't break the Alertmanager Pod outright.
...
# chart's sidecar (observability/values.yaml: grafana.sidecar.dashboards)
# watches for - never a manual Grafana UI import, so reinstalling this
# release from Git alone reproduces every dashboard identically.
