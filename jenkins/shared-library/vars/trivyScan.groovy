// Trivy's scan-then-gate logic, shared between ci/Jenkinsfile's matrix
// Scan+SBOM stage (scanning a pushed image) and pr-Jenkinsfile's quality
// gate (scanning a local Kaniko tarball - PR builds never push anywhere).
// Same two-pass pattern used everywhere else in this project: a full
// HIGH+CRITICAL report first (non-blocking, archived for visibility), then
// the real gate - fixable CRITICAL findings fail the build.
//
...
def call(String target, boolean isTarball, String label) {
    ...
    sh """
        ...
        # --ignore-unfixed excludes CVEs with no available patch; accepted-risk
        # exceptions go in .trivyignore with a documented reason, never a
        # blanket override.
